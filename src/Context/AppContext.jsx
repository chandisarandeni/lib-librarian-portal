  ...
  const login = async (email, password) => {
    try {
      const response = await axios.post(
        "http://localhost:8080/api/v1/librarians/auth/login",
        {
          email,
          password,
        }
        ...
        response.data.map(async (borrowing) => {
          try {
            ...
            const [memberResponse, bookResponse] = await Promise.all([
              axios.get(
                `http://localhost:8080/api/v1/members/${borrowing.memberId}`
              ),
              axios.get(
                `http://localhost:8080/api/v1/books/${borrowing.bookId}`
              ),
            ]);
            ...
            console.error(
              `Error fetching details for borrowing ID ${borrowing.id}:`,
              error
            );
            ...
            try {
              ...
              console.error(
                `Error fetching member details for ID ${borrowing.memberId}:`,
                memberError
              );
            }
            ...
            try {
              ...
              console.error(
                `Error fetching book details for ID ${borrowing.bookId}:`,
                bookError
              );
            }
  ...
  const getRelatedLibrarian = async (email) => {
    try {
      const response = await axios.get(
        `http://localhost:8080/api/v1/librarians/by-email`,
        {
          params: { email },
        }
      );
